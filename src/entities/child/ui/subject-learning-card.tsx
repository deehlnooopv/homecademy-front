 ...
 * - 학습 진도 바 차트
 * - 카드 헤더 클릭 시 AI 분석 패널 펼쳐짐/접힘 (아코디언)
 * - 재능(강점) / 미흡 영역 2단 레이아웃
 * - 어려움을 느끼는 연관 과목 배지
 * - AI 다음 수업 준비 포인트 섹션
...
export function SubjectLearningCard({ card }: SubjectLearningCardProps) {
  ...
  return (
    <Card className="overflow-hidden border border-border/60 hover:shadow-xl transition-all duration-300 bg-card">
      {/* 과목 헤더 - 그라디언트 배경, 클릭 시 AI 분석 패널 토글 */}
      <button
        type="button"
        ...
        <div className={`bg-gradient-to-r ${theme.gradient} p-5 relative overflow-hidden`}>
          ...
          {!isExpanded && (
            <div className="relative z-10 mt-3 flex items-center gap-1.5">
              ...
              <span className="text-white/70 text-xs">클릭하여 AI 분석 보기</span>
            </div>
          )}
        </div>
      </button>
      ...
      {isExpanded && (
        <CardContent className="p-5 space-y-4">
          ...
          <Button
            variant="ghost"
            size="sm"
            className="w-full text-muted-foreground hover:text-foreground text-xs h-7 gap-1"
